...
fn main() {
    ...
    let intersections : Vec<Point> = line1.iter().map(|l| {
        line2.iter().filter_map(move |l2| intersection(l, l2))
    })
    .flatten()
...
fn intersection(a : &Line, b : &Line) -> Option<Point> {
    let (hori, vert) = if a.start.x == a.finish.x { (b,a) } else { (a,b) }; 
    let x = vert.start.x;
...
fn make_lines(l : String) -> Vec<Line> {
    ...
    l.split(',')
        .scan(Point{x:0,y:0}, |state, next|  {
            ...
            Some(Line{start:last,finish:(*state).clone()})
        })
        .collect()
}
